...
# Проверка дали repquota е налична
if ! command -v repquota >/dev/null 2>&1; then
  echo "❌ Липсва команда 'repquota'. Моля, активирайте поддръжката на квоти преди да продължите."
  exit 1
fi

# Сума на всички зададени лимити (block limits) в MB
TOTAL_ALLOCATED_MB=$(sudo repquota -a | awk '$4 ~ /^[0-9]+$/ { sum += $4 } END { print int(sum / 1024) }')
TOTAL_ALLOCATED_MB=${TOTAL_ALLOCATED_MB:-0}

# Изчисляване на свободното пространство по df
...
# Проверка дали остава поне 1 GB
if (( usable_mb < 1024 )); then
  echo "❌ Недостатъчно свободно пространство за създаване на нов хост."
  echo "Свободни: ${available_mb} MB | Заделени чрез квоти: ${TOTAL_ALLOCATED_MB} MB"
  exit 1
fi
...
case "$db_choice" in
  ...
  *)
    ...
    echo "ℹ️ Пропускане на създаването на база данни."
    ;;
esac
