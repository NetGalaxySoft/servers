...
while true; do
  echo "🌐 Реален IP адрес на този сървър: $ACTUAL_IP"
  read -rp "➤ Въведете публичния IP адрес, на който трябва да работи този виртуален хост: " SERVER_IP
  ...
  if [[ "$SERVER_IP" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
    ...
    echo "⚠️ Невалиден IP адрес. Опитайте отново."
  fi
done
...
while true; do
  read -rp "🌐 Въведете основен или субдомейн (напр. example.com или blog.example.com): " input_domain
