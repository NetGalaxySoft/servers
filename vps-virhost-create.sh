...
if mount | grep -q 'on / type' && mount | grep -q 'usrquota'; then
  echo "ℹ️ Опит за активиране на потребителски квоти..."
  quotaon_output=$(sudo quotaon -v -u / 2>&1)

  if echo "$quotaon_output" | grep -q "user quotas turned on"; then
    echo "✅ Потребителските квоти са активни."
  elif echo "$quotaon_output" | grep -q "Device or resource busy"; then
    echo "ℹ️ Потребителските квоти вече са активни."
  else
    ...
    RESULT_USER_QUOTA="⚠️ (активацията се провали)"
    exit 1
  fi
  ...
  if [[ $? -eq 0 ]]; then
    ...
    if [[ "$quota_output" -gt 0 ]]; then
      ...
      echo "⚠️ setquota беше изпълнена, но квотата не е потвърдена от quota -u."
      RESULT_USER_QUOTA="⚠️ (непотвърдено)"
    fi
  else
    echo "❌ Възникна грешка при задаване на квотата."
    RESULT_USER_QUOTA="❌"
  fi
