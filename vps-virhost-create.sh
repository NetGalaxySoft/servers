...
if [[ "$SUMMARY_SSL_TYPE" == "custom" ]]; then
  # === Въвеждане на .crt файл ===
  while true; do
    ...
    if [[ -f "$crt_path" ]]; then
      CUSTOM_CRT_PATH="$crt_path"
      break
    else
      echo "❌ Файлът $crt_path не съществува."
      echo "➤ Изберете:"
      echo "   [1] Опитайте отново"
      echo "   [2] Използвайте Let's Encrypt вместо собствен сертификат"
      echo "   [q] Изход"
      read -rp "Вашият избор [1]: " retry_crt

      case "$retry_crt" in
        2)
          SUMMARY_SSL_TYPE="letsencrypt"
          echo "✅ Превключено към Let's Encrypt (ще се използва certbot)"
          break 2
          ;;
        q|Q)
          echo "🚪 Прекратяване по заявка на оператора."
  ...
  while true; do
    ...
    if [[ -f "$key_path" ]]; then
      ...
      echo "   [2] Използвайте Let's Encrypt вместо собствен сертификат"
      echo "   [q] Изход"
      read -rp "Вашият избор [1]: " retry_key
