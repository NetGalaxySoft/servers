...
while true; do
  ...
  # Проверка за допустим формат на потребителското име
  if [[ ! "$input_user" =~ ^[a-z][a-z0-9_]{2,31}$ ]]; then
    echo "❗ Името трябва да съдържа само малки латински букви, цифри и долна черта (3–32 символа), и да започва с буква."
    continue
  fi
  ...
  if id "$input_user" &>/dev/null; then
    ...
    while true; do
      read -rp "❓ Искате ли да използвате този съществуващ потребител? [y/n/q]: " confirm
      case "$confirm" in
        y|Y ) break 2 ;;
        n|N ) break ;;
        q|Q ) echo "🚪 Прекратяване по заявка на оператора."; exit 0 ;;
        *    ) echo "⚠️ Моля, отговорете с 'y', 'n' или 'q'." ;;
      esac
    done
  else
    break
...
# Въвеждане на парола с проверки
while true; do
  ...
  if ! [[ "$pass1" =~ [0-9] ]]; then
    echo "❗ Паролата трябва да съдържа поне една цифра."
    continue
  fi
  ...
  if [[ "$pass1" =~ [^[:ascii:]] ]]; then
    echo "❗ Паролата съдържа символи извън латиницата. Моля, използвайте само латински букви и символи."
    continue
  fi
