...
# === [7] СЪЗДАВАНЕ НА ПРОФИЛ ЗА ХОСТИНГ МЕНИДЖЪР ==================
echo ""
echo "[7] СЪЗДАВАНЕ НА ПРОФИЛ ЗА ХОСТИНГ МЕНИДЖЪР..."
echo "-------------------------------------------------------------------------"
echo ""
enecho "👤 Създаване на профил за мениджъра на този хост."
...
while true; do
  ...
  if id "$input_user" &>/dev/null; then
    # === [7.1] СЪЩЕСТВУВАЩ ПОТРЕБИТЕЛ ===
    echo "⚠️ Потребителят '$input_user' вече съществува."
    while true; do
      read -rp "❓ Искате ли да използвате този съществуващ потребител? [y/n/q]: " reuse
      case "$reuse" in
        y|Y)
          # === [7.2] ДОБАВЯНЕ КЪМ ГРУПАТА ===
          if id "$input_user" | grep -q "grp_${domain_clean}"; then
            echo "ℹ️ Потребителят вече е член на групата grp_${domain_clean}."
          else
            sudo usermod -a -G "grp_${domain_clean}" "$input_user"
            echo "✅ Потребителят е добавен в групата grp_${domain_clean}."
          fi
          SUMMARY_ADMIN_USER="$input_user"
          SUMMARY_ADMIN_PASS="(съществуваща парола — неизвестна за системата)"
          break 2
          ;;
        n|N) break ;;  # Ще поиска ново име
        q|Q) echo "🚪 Прекратяване по заявка на оператора." && exit 0 ;;
        *) echo "❗ Моля, отговорете с y (да), n (не) или q (изход)." ;;
      esac
    done
  else
    # === [7.3] СЪЗДАВАНЕ НА НОВ ПОТРЕБИТЕЛ ===
    ...
    case "$confirm_pass" in
      y|Y)
        ...
        echo "$input_user:$ONE_TIME_PASS" | sudo chpasswd
        sudo chage -d 0 "$input_user"  # изискване за смяна на парола при първо влизане
        sudo usermod -a -G "grp_${domain_clean}" "$input_user"
        SUMMARY_ADMIN_USER="$input_user"
        SUMMARY_ADMIN_PASS="$ONE_TIME_PASS"
        echo "✅ Нов потребител е създаден и добавен в grp_${domain_clean}: $SUMMARY_ADMIN_USER"
        break
        ;;
      q|Q)
...
echo ""
echo "✅ Скриптът приключи успешно."
echo "========================================================================="
