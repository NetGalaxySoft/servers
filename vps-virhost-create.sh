...
while true; do
  ...
  case "$ssl_choice" in
    ...
    2)
      ...
      while true; do
        read -rp "📄 Въведете пълния път до .crt файла: " custom_crt
        [[ "$custom_crt" == "q" ]] && exit 0
        [[ ! -f "$custom_crt" ]] && {
          echo "❌ Файлът $custom_crt не съществува. Опитайте отново."
          continue
        }

        read -rp "🔑 Въведете пълния път до .key файла: " custom_key
        [[ "$custom_key" == "q" ]] && exit 0
        [[ ! -f "$custom_key" ]] && {
          echo "❌ Файлът $custom_key не съществува. Опитайте отново."
          continue
        }

        SUMMARY_SSL_CERT="$custom_crt"
        SUMMARY_SSL_KEY="$custom_key"
        echo "✅ Сертификат: $custom_crt"
        echo "✅ Ключ: $custom_key"
        break
      done
      break
      ;;
