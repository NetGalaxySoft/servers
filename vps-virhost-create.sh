  ...
  while true; do
        ...
        while true; do
          ...
          if [[ $? -eq 0 ]]; then
            ...
            while true; do
              ...
              case "$retry" in
                p|P) break ;;         # ще повтори въвеждане на паролата
                n|N) continue 3 ;;    # ще върне към начално въвеждане на потребител
    ...
    while true; do
      ...
      if [[ "$pass1" != "$pass2" ]]; then
        ...
        SUMMARY_ADMIN_USER="$input_user"
        SUMMARY_ADMIN_PASS="$pass1"
        echo "✅ Ще бъде създаден нов потребител: $SUMMARY_ADMIN_USER"
        break 2
      fi
    done
  fi
