...
while true; do
  ...
  # Извличане на публичния IP на сървъра
  ...
  # Проверка дали домейнът се резолвира
  ...
  if [[ "$resolved_ip" != "$real_ip" ]]; then
    echo "❌ Домейнът се резолвира към $resolved_ip, но публичният IP адрес на сървъра е $real_ip."
    echo "🛑 Уверете се, че DNS записите сочат към този сървър."
    continue
  fi

  echo "✅ Домейнът $input_domain се резолвира правилно към $real_ip"
  break
done
