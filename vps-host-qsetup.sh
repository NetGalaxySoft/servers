...
# Инсталиране на необходимия пакет за работа с квоти
...
echo ""
echo "✅ Квотите са конфигурирани в /etc/fstab."
echo "🔁 Необходим е рестарт, за да бъдат монтирани."
...
# Проверка дали root файловата система е монтирана с активирани квоти
if [[ "${RESULT_QUOTAS}" == "✅" ]]; then
  if ! mount | grep -E 'on / type' | grep -q 'usrquota' || ! mount | grep -E 'on / type' | grep -q 'grpquota'; then
    echo ""
    echo "⚠️  ВНИМАНИЕ: Квотите бяха конфигурирани, но root файловата система все още не е монтирана с нужните опции."
    echo "🔁 Необходим е рестарт на системата, за да се активират квотите правилно."

    # Предложение за рестарт
