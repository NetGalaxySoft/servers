...
if sudo apt-get install -y quota; then
  QUOTA_INSTALLED="yes"
else
  echo "❌ Неуспешна инсталация на 'quota'. Прекъсване."
  RESULT_QUOTAS="❌"
  exit 1
fi

# Проверка за наличие на usrquota,grpquota във fstab
...
# Ако всичко премина успешно – задаваме резултат
RESULT_QUOTAS="✅"

echo ""
echo "✅ Квотите са конфигурирани в /etc/fstab."
echo "🔁 Необходим е рестарт, за да бъдат монтирани правилно."
