...
# === [МОДУЛ 2] ИНСТАЛИРАНЕ НА APACHE =======================================
echo "[2] ИНСТАЛИРАНЕ НА APACHE..."
echo "----------------------------------------------------------------------"

MODULE_NAME="host_02_apache_check"
SETUP_ENV_FILE="/etc/netgalaxy/setup.env"
...
if grep -q "^RESULT_HOST_APACHE_CHECK=✅" "$SETUP_ENV_FILE"; then
  ...
  echo "⏳ Инсталиране на пакета apache2..."
  if sudo apt-get update -qq && sudo apt-get install -y apache2; then
    echo "✅ Apache беше инсталиран успешно."
    RESULT_HOST_APACHE_CHECK="✅"
  else
    echo "❌ Грешка при инсталиране на Apache."
    echo "Изпълнението на скрипта не може да продължи."
    echo "Моля, отстранете проблема ръчно и стартирайте отново този скрипт."
    echo "RESULT_HOST_APACHE_CHECK=❌" | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
    exit 1
  fi
