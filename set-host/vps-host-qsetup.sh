...
if [[ ! -f "$SETUP_ENV_FILE" ]] || ! grep -q '^SETUP_VPS_BASE_STATUS=✅' "$SETUP_ENV_FILE"; then
  echo "🛑 Не е открита начална конфигурация на сървъра!"
  echo "   Скриптът не може да продължи, защото:"
  echo "   - Или сървърът не е бил конфигуриран със скрипта vps-base-qsetup.sh"
  echo "   - Или конфигурацията е била премахната или нарушена"
  echo ""
  echo "🔧 Моля, изпълнете първо началния скрипт: vps-base-qsetup.sh"
  echo ""
  [[ -f "$0" ]] && rm -- "$0"
  exit 1
fi
