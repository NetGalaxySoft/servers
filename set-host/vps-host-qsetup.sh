...
if grep -q "^$MODULE_NAME\b" "$MODULES_FILE"; then
  ...
  while true; do
    ...
    if [[ "$ACTUAL_DOMAIN" != "$SERVER_DOMAIN" ]]; then
      echo ""
      echo "🚫 Скриптът е стартиран на сървър с различно домейн име."
      echo "⚠️ Несъответствие! Въведеният FQDN не съвпада с текущото име на машината."
      echo ""
      read -p "🔁 Искате ли да опитате отново? [Enter за ДА, 'q' за изход]: " retry
      if [[ "$retry" == "q" || "$retry" == "Q" ]]; then
        echo "⛔ Скриптът беше прекратен от потребителя след $MODULE_NAME."
      ...
      echo "✅ Потвърдено: текущото име на сървъра съвпада с въведения FQDN ($SERVER_DOMAIN)."
      break
    fi
  done

  # ✅ Записване на стойностите в setup.env
  {
    echo "SERVER_IP=\"$SERVER_IP\""
    echo "SERVER_DOMAIN=\"$SERVER_DOMAIN\""
  } | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
