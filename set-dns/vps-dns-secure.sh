...
if sudo grep -q '^SECURE_DNS_MODULE6=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  # -------------------------------------------------------------------------------------
  # СЕКЦИЯ 2: Зареждане на данни от todo.modules
  # -------------------------------------------------------------------------------------
  if [[ ! -f "$MODULES_FILE" ]]; then
  ...
  if echo "$DIG_RESULT" | grep -q "Transfer failed"; then
    ...
    echo "❌ Връзката е неуспешна (timeout). Проверете firewall и достъп между сървърите."
    exit 1
  elif echo "$DIG_RESULT" | grep -q "TSIG error"; then
    echo "❌ TSIG грешка! Провалена автентикация. Проверете ключовете на двата сървъра."
    exit 1
  elif echo "$DIG_RESULT" | grep -q "IN"; then
    echo "✅ TSIG тестът е успешен! Зонен трансфер е разрешен и удостоверен."
  else
    echo "⚠️ Резултатът е неочакван. Проверете ръчно:"
    echo "$DIG_RESULT"
    exit 1
  fi
