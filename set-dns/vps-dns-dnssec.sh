...
if sudo grep -q '^DNSSEC_MODULE2=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  # -------------------------------------------------------------------------------------
  # СЕКЦИЯ 2: Откриване на зони
  # -------------------------------------------------------------------------------------
  echo "🔍 Проверка на конфигурираните зони..."
  ZONES=$(grep -E 'zone\s+"[a-zA-Z0-9.-]+"' /etc/bind/named.conf.local | awk '{print $2}' | tr -d '";')

  if [[ -z "$ZONES" ]]; then
    echo "❌ Не са открити конфигурирани зони. Прекратяване."
    exit 1
  fi
  ...
  # Взимаме първата зона за подписване (основния домейн)
  DOMAIN=$(echo "$ZONES" | head -n 1)

  if [[ -z "$DOMAIN" ]]; then
    echo "❌ Не може да се определи основната зона. Прекратяване."
    exit 1
  fi
  ...
  # -------------------------------------------------------------------------------------
  cd "$DNSSEC_DIR" || { echo "❌ Неуспешен достъп до $DNSSEC_DIR"; exit 1; }

  # KSK (Key Signing Key)
  if ! sudo dnssec-keygen -a RSASHA256 -b 2048 -f KSK "$DOMAIN"; then
    echo "❌ Грешка при генериране на KSK!"
    exit 1
  fi

  # ZSK (Zone Signing Key)
  if ! sudo dnssec-keygen -a RSASHA256 -b 1024 "$DOMAIN"; then
    echo "❌ Грешка при генериране на ZSK!"
    exit 1
  fi

  echo "✅ Ключовете са генерирани успешно в $DNSSEC_DIR"
  echo ""
