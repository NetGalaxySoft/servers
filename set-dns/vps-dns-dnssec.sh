...
if sudo grep -q '^DNSSEC_MODULE2=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  # -------------------------------------------------------------------------------------
  # СЕКЦИЯ 1: Подготовка на директория за DNSSEC ключове
  # -------------------------------------------------------------------------------------
  ...
  # -------------------------------------------------------------------------------------
  # СЕКЦИЯ 2: Проверка на зони
  # -------------------------------------------------------------------------------------
  echo "🔍 Проверка на конфигурираните зони..."
  ZONES_FILE="/etc/bind/named.conf.local"
  if [[ ! -f "$ZONES_FILE" ]]; then
    echo "❌ Липсва файлът $ZONES_FILE. Скриптът не може да продължи."
    exit 1
  fi

  # Извличане на зоните от named.conf.local
  ZONES=$(grep 'zone "' "$ZONES_FILE" | awk '{print $2}' | tr -d '"')
  if [[ -z "$ZONES" ]]; then
    echo "❌ Не са открити зони в $ZONES_FILE."
    exit 1
  fi

  echo "✅ Засечена зона: $ZONES"
  for zone in $ZONES; do
    ZONE_FILE=$(grep -A 2 "zone \"$zone\"" "$ZONES_FILE" | grep 'file' | awk '{print $2}' | tr -d '";')
    echo "$zone (файл: $ZONE_FILE)"
  done
  echo ""

# -------------------------------------------------------------------------------------
# СЕКЦИЯ 3: Генериране на DNSSEC ключове (KSK и ZSK)
# -------------------------------------------------------------------------------------
DOMAIN=$(echo "$ZONES" | head -n 1)  # Вземаме първата зона като основна
echo "🔐 Генериране на KSK и ZSK за $DOMAIN..."

# Генериране на ключове директно в DNSSEC_DIR
KSK_KEY=$(sudo dnssec-keygen -a ECDSAP256SHA256 -f KSK -b 2048 -K "$DNSSEC_DIR" "$DOMAIN")
ZSK_KEY=$(sudo dnssec-keygen -a ECDSAP256SHA256 -b 2048 -K "$DNSSEC_DIR" "$DOMAIN")

if [[ -n "$KSK_KEY" && -n "$ZSK_KEY" ]]; then
  echo "✅ Генерирани ключове:"
  echo "KSK: $KSK_KEY"
  echo "ZSK: $ZSK_KEY"
else
  echo "❌ Грешка при генериране на ключове!"
  exit 1
fi
echo ""

  # -------------------------------------------------------------------------------------
  # СЕКЦИЯ 4: Задаване на правилни права на файловете
  # -------------------------------------------------------------------------------------
  echo "🔧 Настройка на права за ключовите файлове..."
  sudo chown bind:bind "$DNSSEC_DIR"/*
  sudo chmod 640 "$DNSSEC_DIR"/*
  echo "✅ Права на ключовете са коректни."
  echo ""
