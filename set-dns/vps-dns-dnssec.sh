...
if sudo grep -q '^DNSSEC_MODULE2=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  for ZONE in $ZONES; do
    ...
    if ! sudo dnssec-keygen -a RSASHA256 -b 2048 -f KSK "$ZONE" -K "$DNSSEC_DIR" >/dev/null 2>&1; then
      echo "❌ Грешка при генериране на KSK за $ZONE!"
      exit 1
    fi

    # Генериране на ZSK (Zone Signing Key)
    if ! sudo dnssec-keygen -a RSASHA256 -b 1024 "$ZONE" -K "$DNSSEC_DIR" >/dev/null 2>&1; then
      echo "❌ Грешка при генериране на ZSK за $ZONE!"
      exit 1
    fi

    echo "✅ KSK и ZSK са генерирани успешно за $ZONE."
    echo ""
  done
