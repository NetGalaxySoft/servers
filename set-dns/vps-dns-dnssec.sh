...
if sudo grep -q '^DNSSEC_MODULE2=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  # -------------------------------------------------------------------------------------
  # СЕКЦИЯ 1: Подготовка на директорията за ключове
  # -------------------------------------------------------------------------------------
  ...
  # Избираме основната зона (първата в списъка)
  DOMAIN=$(echo "$ZONES" | head -n 1)
  if [[ -z "$DOMAIN" ]]; then
    echo "❌ Не е намерена нито една зона в конфигурацията."
    exit 1
  fi
...
# Проверка на достъп до директорията
if [[ ! -d "$DNSSEC_DIR" ]]; then
  echo "❌ Директорията $DNSSEC_DIR не съществува."
  exit 1
fi

# Генериране на KSK
if sudo dnssec-keygen -a RSASHA256 -b 2048 -f KSK -K "$DNSSEC_DIR" "$DOMAIN"; then
  echo "✅ KSK е генериран успешно."
else
...
# Генериране на ZSK
if sudo dnssec-keygen -a RSASHA256 -b 1024 -K "$DNSSEC_DIR" "$DOMAIN"; then
  echo "✅ ZSK е генериран успешно."
else
  echo "❌ Грешка при генериране на ZSK за $DOMAIN!"
  exit 1
fi

# Присвояване на права
sudo chown bind:bind "$DNSSEC_DIR"/*
