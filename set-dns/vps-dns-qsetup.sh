...
# ✅ Проверка за съществуване на todo.modules (ако съществува → грешка)
if [[ -f "$MODULES_FILE" ]]; then
  echo "🛑 Открит файл $MODULES_FILE. Стандартната начална конфигурация не е завършена."
  echo "❗ Моля, завършете тази конфигурация и опитайте отново."
  echo "🗑️ Премахване на скрипта."
  [[ -f "$0" ]] && rm -- "$0"
  exit 1
fi
...
# ✅ Добавена проверка дали стойностите са непразни
if [[ -z "$OS_NAME" || -z "$OS_VERSION" ]]; then
  echo "❌ Неуспешно извличане на информация за ОС. Файлът /etc/os-release може да е повреден."
  exit 1
fi
...
while true; do
  ...
  # Проверка дали въведеният IP е валиден IPv4
  ...
  # ✅ Добавена проверка за валидност на ACTUAL_IP
  if ! [[ "$ACTUAL_IP" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "❌ Неуспех при извличане на реалния IP адрес. Проверете интернет връзката."
    exit 1
  fi

  # Сравнение на IP адресите
  if [[ "$ACTUAL_IP" != "$SERVER_IP" ]]; then
    echo "🚫 Несъответствие! Въведеният IP ($SERVER_IP) не съвпада с реалния IP ($ACTUAL_IP)."
...
# ✅ По-строга проверка: ns1/ns2/ns3 + минимум два сегмента (example.com)
if [[ ! "$HOSTNAME_FQDN" =~ ^ns[1-3]\.[a-zA-Z0-9-]+\.[a-zA-Z]{2,}$ ]]; then
  echo "🚫 Несъвместим или недопустим домейн: $HOSTNAME_FQDN"
  echo "❗ FQDN трябва да бъде във формат ns1.example.com, ns2.example.com или ns3.example.com"
  exit 1
fi
