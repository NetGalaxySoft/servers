...
cleanup_and_exit() {
  echo "🗑️ Премахване на временните файлове..."
  [[ -f "$MODULES_FILE" ]] && sudo rm -f "$MODULES_FILE"
  [[ -f "$0" ]] && rm -- "$0"
  exit "$1"
}
...
if sudo grep -q '^SETUP_VPS_DNS_STATUS=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  echo "🛑 Скриптът за конфигуриране на DNS сървър вече е изпълнен. Повторното изпълнение може да срине системата."
  cleanup_and_exit 0
fi
...
if sudo grep -q '^DNS_RESULT_MODULE1=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  while true; do
    ...
    if [[ "$ACTUAL_IP" != "$SERVER_IP" ]]; then
      ...
      [[ "$retry" == "q" || "$retry" == "Q" ]] && cleanup_and_exit 0
      echo ""
    else
