...
if [[ ! -d "$NETGALAXY_DIR" ]]; then
  ...
  echo "ℹ️ Открита съществуваща директория: $NETGALAXY_DIR"

  # Проверка дали конфигурацията вече е била завършена
  if [[ -f "$SETUP_ENV_FILE" ]]; then
    if grep -q '^SETUP_VPS_BASE_DONE=✅' "$SETUP_ENV_FILE"; then
      echo "🛑 Конфигурацията вече е завършена и потвърдена от оператор."
      echo "   Скриптът няма да бъде изпълнен повторно, за да не наруши работеща система."
...
read -rp "Вашият избор (y/n): " final_choice
...
if [[ "$final_choice" =~ ^[Yy]$ ]]; then
  ...
  sudo sed -i '/^SETUP_VPS_BASE_DONE=/d' "$SETUP_ENV_FILE" 2>/dev/null
  sudo sed -i '/^SETUP_VPS_BASE_STATUS=/d' "$SETUP_ENV_FILE" 2>/dev/null
  ...
  sudo rm -f "$SETUP_ENV_FILE" "$MODULES_FILE"
  echo "🔄 Рестартиране на системата след 3 секунди..."
  sleep 3
  # 🗑️ Самоунищожаване на скрипта (ако е стартиран от файл)
  if [[ -f "$0" ]]; then
    sudo rm -- "$0"
  fi
  sudo reboot
