...
# --- [5] ПРОВЕРКА НА FIREWALL СИСТЕМА --------------------------------------------
echo "[5] ПРОВЕРКА НА FIREWALL СИСТЕМА..."
echo "-------------------------------------------------------------------------"
echo ""
...
if [[ "$FIREWALL_SYSTEM" == "ufw" ]]; then
  ...
  # Проверка без активиране – безопасно извличане
  if sudo ufw status | grep -iq "inactive"; then
    echo "ℹ️  UFW е инсталиран, но не е активиран."
  else
    echo "ℹ️  UFW изглежда активен – ще покажем статуса:"
    sudo ufw status numbered 2>/dev/null | sed 's/^/    /'
  fi

elif [[ "$FIREWALL_SYSTEM" == "none" ]]; then
  echo "⚠️  В системата не е открита инсталирана защитна стена."
else
  echo "🔁 В момента използвате защитна стена: $FIREWALL_SYSTEM"
  echo "📌 В бъдеще ще използвате защитна стена **UFW**, която е задължителна за сървърите на платформата NetGalaxy."
  echo ""
  echo "📖 Засечени отворени портове в $FIREWALL_SYSTEM:"
  ss -tuln | awk 'NR>1 {print $5}' | cut -d: -f2 | sort -nu | sed 's/^/  - /'
fi
echo ""
echo ""

# --- [5.1] ИНФОРМАЦИЯ ЗА ИЗПОЛЗВАНЕ НА UFW ----------------------------------------
echo "[5.1] ИНФОРМАЦИЯ ЗА ИЗПОЛЗВАНЕ НА UFW..."
echo "-------------------------------------------------------------------------"
echo ""

if [[ "$FIREWALL_SYSTEM" == "none" ]]; then
  echo "⚠️  Вашата система в момента **не използва защитна стена**."
  echo "🛡️  Ще бъде инсталирана и активирана защитна стена **UFW**, която е задължителна за всички сървъри на платформата NetGalaxy."
  echo ""
fi
echo ""

# --- [5.2] ВЪВЕЖДАНЕ НА ДОПЪЛНИТЕЛНИ FIREWALL ПОРТОВЕ ----------------------------
echo "[5.2] ВЪВЕЖДАНЕ НА ДОПЪЛНИТЕЛНИ FIREWALL ПОРТОВЕ..."
echo "-------------------------------------------------------------------------"
echo ""
...
if [[ "$FIREWALL_SYSTEM" != "none" ]]; then
  ...
  if [[ -n "$SSH_PORT" ]]; then
    echo ""
    echo "🔐 SSH порт, избран за достъп до сървъра: $SSH_PORT"
  fi
...
if [[ "$FIREWALL_SYSTEM" == "none" || "$open_more" == "y" || "$open_more" == "Y" ]]; then
  echo ""
  echo "🧩 Въведете портове, които желаете да бъдат отворени в защитната стена (UFW):"
  while true; do
    printf "➤ Въведете порт (Enter за край, q за прекратяване): "
    read port
