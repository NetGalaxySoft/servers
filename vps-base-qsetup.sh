...
if [[ "$FIREWALL_SYSTEM" != "none" ]]; then
  ...
  while true; do
    ...
    if [[ "$add_ports" == "q" || "$add_ports" == "Q" ]]; then
      echo "❎ Скриптът беше прекратен от потребителя."
      exit 0
    elif [[ "$add_ports" == "y" || "$add_ports" == "Y" ]]; then
      break
    elif [[ -z "$add_ports" ]]; then
      echo "🔒 Пропускане на добавяне на нови портове."
      exit 0
    else
      echo "❌ Невалиден отговор. Въведете 'y', Enter или 'q'."
    fi
  done
fi

# Въвеждане на портове
echo ""
