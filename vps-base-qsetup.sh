...
# --- [5.1] ОТВОРЕНИ FIREWALL ПОРТОВЕ И ИЗИСКВАНИЯ НА ПЛАТФОРМАТА ------------------
echo "[5.1] ОТВОРЕНИ FIREWALL ПОРТОВЕ И ИЗИСКВАНИЯ НА ПЛАТФОРМАТА..."
echo "-------------------------------------------------------------------------"
echo ""
...
# --- [5.2] ДОБАВЯНЕ НА ДОПЪЛНИТЕЛНИ FIREWALL ПОРТОВЕ -----------------------------
echo "[5.2] ДОБАВЯНЕ НА ДОПЪЛНИТЕЛНИ FIREWALL ПОРТОВЕ..."
echo "-------------------------------------------------------------------------"
echo ""

PORT_LIST=()

# Показване само ако има активна защитна стена
if [[ "$FIREWALL_SYSTEM" != "none" ]]; then
  echo "📡 Засечени отворени портове в системата:"
  if [[ "$FIREWALL_SYSTEM" == "ufw" ]]; then
    sudo ufw status numbered 2>/dev/null | sed 's/^/  - /'
  else
    ss -tuln | awk 'NR>1 {print $5}' | cut -d: -f2 | sort -nu | sed 's/^/  - /'
  ...
  while true; do
    printf "➕ Желаете ли да добавите нови портове за отваряне? (y за да добавите, Enter за пропускане, q за прекратяване): "
    read open_more
...
# Реално въвеждане на нови портове (ако няма стена или операторът е избрал "y")
if [[ "$FIREWALL_SYSTEM" == "none" || "$open_more" == "y" || "$open_more" == "Y" ]]; then
  echo ""
  echo "🧩 Въведете портовете, които желаете да бъдат отворени в защитната стена (UFW):"
  echo "(Натиснете Enter без въвеждане, когато приключите. 'q' за прекратяване)"
  while true; do
