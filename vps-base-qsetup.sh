...
# --- [5.3] ВЪВЕЖДАНЕ НА ДОВЕРЕНИ МРЕЖИ ----------------------------------------
echo "[5.3] ВЪВЕЖДАНЕ НА ДОВЕРЕНИ МРЕЖИ ЗА UFW..."
echo "-------------------------------------------------------------------------"
echo ""
...
echo "👉 Въведете по един CIDR адрес на ред (напр. 192.168.1.0/24)."
echo "👉 Натиснете Enter без въвеждане за край или 'q' за прекратяване."
echo ""

while true; do
  printf "➤ Въведете CIDR на доверена мрежа: "
  read -r net
  if [[ "$net" == "q" || "$net" == "Q" ]]; then
    echo "❎ Скриптът беше прекратен от потребителя."
    exit 0
