...
if [[ "$ACTUAL_IP" != "$SERVER_IP" ]]; then
  echo ""
  echo "🚫 Скриптът може би не е стартиран на правилната машина!"
  echo "👉 Въведен IP адрес: $SERVER_IP"
  echo "📍 Открит IP адрес:  $ACTUAL_IP"
  echo "⚠️ Адресите не съвпадат."
  echo ""
  read -p "❓ Искате ли въпреки това да продължите с изпълнението? [Enter за ДА, 'q' за прекратяване]: " confirm
