...
echo "✅ Сървърът ще бъде конфигуриран със следните параметри:"
echo -e " - \033[1mДомейн (FQDN):\033[0m             $FQDN"
echo -e " - \033[1mSSH порт:\033[0m                  $SSH_PORT"
echo -e " - \033[1mАдмин потребител:\033[0m          $ADMIN_USER"
echo -e " - \033[1mRoot достъп по SSH:\033[0m        ще бъде забранен"
echo -e " - \033[1mIP адрес на сървъра:\033[0m       $SERVER_IP"
echo -e " - \033[1mЗащитна стена:\033[0m             $FIREWALL_SYSTEM"

# Допълнителни портове (ако има)
if [[ ${#PORT_LIST[@]} -gt 0 ]]; then
  echo -e " - \033[1mДопълнителни портове:\033[0m       ${PORT_LIST[*]}"
fi

# Частни мрежи (ако има въведени)
if [[ ${#TRUSTED_NETS[@]} -gt 0 ]]; then
  echo -e " - \033[1mДоверени мрежи (VPN/LAN):\033[0m  ${TRUSTED_NETS[*]}"
else
  echo -e " - \033[1mДоверени мрежи (VPN/LAN):\033[0m  няма"
fi
...
while true; do
  printf "✔️ Въведете \"y\" за продължение или \"q\" за прекратяване: "
  read -r CONFIRMATION
