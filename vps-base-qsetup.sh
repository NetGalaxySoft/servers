...
if [[ "$FIREWALL_SYSTEM" != "none" ]]; then
  ...
  echo ""
  echo "🧩 Въведете по един порт на ред (между 1 и 65535)."
  echo "👉 Натиснете Enter без въвеждане, когато приключите."
  echo "👉 Въведете 'q' за прекратяване на скрипта."
  echo ""
...
while true; do
  printf "🌐 Ще използвате ли достъп от частна мрежа (напр. VPN, вътрешна LAN)? (y / Enter за пропускане / q за прекратяване): "
  ...
  elif [[ -z "$use_private_net" ]]; then
    echo "🔒 Пропускане на добавяне на доверени частни мрежи."
    break

  else
    echo "❌ Невалиден отговор. Моля, въведете 'y', Enter или 'q'."
  fi
done
echo ""
echo ""
...
# Проверка дали терминалът поддържа цветове и форматиране
...
echo -e "✅ ${BOLD_BLUE}Сървърът ще бъде конфигуриран със следните параметри:${RESET}"
echo ""

echo -e " - ${BOLD_BLUE}Домейн (FQDN):${RESET} ${BOLD_GREEN}$FQDN${RESET}"
echo -e " - ${BOLD_BLUE}SSH порт:${RESET} ${BOLD_GREEN}$SSH_PORT${RESET}"
echo -e " - ${BOLD_BLUE}Админ потребител:${RESET} ${BOLD_GREEN}$ADMIN_USER${RESET}"
echo -e " - ${BOLD_BLUE}Root достъп по SSH:${RESET} ${BOLD_GREEN}ще бъде забранен${RESET}"

echo -e " - ${BOLD_BLUE}IP адрес на сървъра:${RESET} ${BOLD_GREEN}$SERVER_IP${RESET}"
echo -e " - ${BOLD_BLUE}Защитна стена:${RESET} ${BOLD_GREEN}$FIREWALL_SYSTEM${RESET}"

if [[ ${#PORT_LIST[@]} -gt 0 ]]; then
  echo -e " - ${BOLD_BLUE}Допълнителни портове за отваряне:${RESET} ${BOLD_GREEN}${PORT_LIST[*]}${RESET}"
fi

if [[ ${#TRUSTED_NETS[@]} -gt 0 ]]; then
  echo -e " - ${BOLD_BLUE}Доверени мрежи:${RESET} ${BOLD_GREEN}${TRUSTED_NETS[*]}${RESET}"
else
  echo -e " - ${BOLD_BLUE}Доверени мрежи:${RESET} ${BOLD_GREEN}няма${RESET}"
fi

echo ""
echo -e "🧐 ${BOLD_BLUE}Моля, прегледайте внимателно правилността на въведената информация по-горе.${RESET}"
echo -e "Ако имате забележки, прекратете конфигурацията, коригирайте и стартирайте отново този скрипт."
echo ""
...
echo "[15] ОБОБЩЕНИЕ НА РЕЗУЛТАТИТЕ ОТ КОНФИГУРАЦИЯТА"
echo "-------------------------------------------------------------------------"
