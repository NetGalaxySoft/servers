...
echo "   - Поне една латинска буква (a-z, A-Z)"
...
while true; do
  ...
  # Проверка за дължина, буква и цифра
  if (( ${#PASSWORD_1} < 8 )) || \
     ! [[ "$PASSWORD_1" =~ [A-Za-z] ]] || \
     ! [[ "$PASSWORD_1" =~ [0-9] ]]; then
    echo "❌ Паролата трябва да съдържа поне 8 символа, включително латинска буква и цифра."
    continue
  fi

  # Проверка за символи извън латиницата
  if [[ "$PASSWORD_1" =~ [^[:ascii:]] ]]; then
    echo "⚠️ Внимание: Открити са букви или знаци, които не са част от стандартната латиница."
