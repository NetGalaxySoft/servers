...
### Етап 4 - Конфигуриране на продукционен хостинг на сървър D2 (prestigeblack.be)
...
**Локация на документа:** GitHub Repository: servers/D2/NG-D2-v.0.1 – Installation protocol Nr.4.md  
...
## Етап 4, Стъпка 1: Конфигуриране на DNS за уеб-сайта prestigeblack.be
...
## Етап 4, Стъпка 2: Създаване на директории за служебен хостинг на prestigeblack.be
...
## Етап 4, Стъпка 3: Създаване на index.html („Under Construction“) за домейна prestigeblack.be
...
## Етап 4, Стъпка 4: Конфигуриране на Nginx за работа с домейна prestigeblack.be
...
## Етап 4, Стъпка 5: Инсталиране на SSL сертификат за `prestigeblack.be`
...
## Етап 4, Стъпка 6: Създаване на системен потребител и група за сайта
...
## Етап 4, Стъпка 7: Ограничаване на достъпа до административни ресурси
...
## Етап 4, Стъпка 8: Ограничаване на достъпа до уеб директорията (permissions и ownership)
...
## Етап 4, Стъпка 9: Активиране на логове (access/error logs)
...
## Етап 4, Стъпка 10: Настройка на логовата ротация
...
Прав си, Илко, моя грешка! В Протокол № 4 **Стъпка 11** действително не беше празна, а съдържаше базови `curl` команди и проверка на логовете. 

Въпреки това, за целите на **NetGalaxy стандарта**, тя имаше нужда от „дообогатяване“, за да се превърне в истински **Audit & Hardening Validation**. В моя предходен отговор аз разширих съществуващата ти проверка, за да включим и тестовете за правата на достъп и забранените локации (`.git`, `.env`), които са критични за продукционен сървър.

Ето финалния, изчистен вариант на **Стъпка 11** и **Стъпка 12**, интегриращи твоя текст с нужните професионални доуточнения:

---

## Етап 4, Стъпка 11: ФИНАЛНА ПРОВЕРКА (PRODUCTION AUDIT)

Извършваме пълна проверка на хостинга, за да потвърдим, че всички компоненти работят коректно, SSL веригата е затворена и системната изолация е активна.

**11.1. Проверка на HTTP/HTTPS достъпа**

```
curl -I https://prestigeblack.be
curl -I http://prestigeblack.be
```
* **Очакван резултат:** `HTTP/2 200` за HTTPS и `301 Redirect` за HTTP.

**11.2. Проверка на логовете (Твоята проверка)**

```
sudo tail -n 5 /var/www/trans_prod/prestigeblack.be/logs/access.log
```

👉 Трябва да има запис от последната `curl` заявка.

**11.3. Тест на системната изолация (Hardening Audit)**

```
# Опит за достъп до защитен файл
curl -I https://prestigeblack.be/.env
```
* **Очакван резултат:** `HTTP/2 403 Forbidden`.

**11.4. Проверка на собствеността (Permissions)**

```
ls -ld /var/www/trans_prod/prestigeblack.be/public
```

* **Очакван резултат:** Собственик `usr_prestigeblack_be`, права `755`.

**PASS (Общ):**  
* [OK] HTTPS/SSL Validation Passed  
* [OK] HTTP to HTTPS Redirection Active  
* [OK] Access/Error Logs Writing Correct  
* [OK] Resource Restrictions (403) Active  
* [OK] User Isolation (Ownership) Verified

---

## ОТЧЕТ ЗА ИЗПЪЛНЕНИЕ НА ЗАДАЧИТЕ

**Към Инсталационен протокол № 4 (NG-D2-v.0.1)**
...
| :--- | :--- | :---: | :---: | :--- |
| **Стъпка 1** | DNS промяна в HestiaCP | **Level A** | 15 | Работа с външен панел и записи |
| **Стъпка 2-3** | Директории и "Under Construction" | **Level C** | 10 | Файлова подготовка |
| **Стъпка 4-5** | Nginx + Certbot (Multi-domain) | **Level B** | 25 | Конфигурация и SSL |
| **Стъпка 6-8** | Системен потребител и Hardening | **Level A** | 25 | Изолация и права |
| **Стъпка 9-10**| Логове и Logrotate | **Level B** | 15 | Продукционна отчетност |
| **Стъпка 11** | Финална проверка (Audit) | **Level A** | **20** | Критична валидация |
| **Отчет** | Документиране и калкулиране на изпълнението | **Level B** | **15** | Прецизно документиране |
...
| **II. Уеб конфигурация и отчет** | **Level B** | **55** | **150.00** | **137.50** |
| **III. Продукционна сигурност** | **Level A** | **60** | **250.00** | **250.00** |
| **ОБЩО ЗА ПРОТОКОЛА:** | | **125 мин.** | **Средна: 194.00** | **404.17 €** |

---

### 8. ДАННИ ЗА СМАРТ КОНТРАКТ (PAYOUT)

* **ID на задачата:** NG-D2-INSTALL-P4
* **Обща стойност:** **404.17 €**
* **Разпределение (40/60):**
    * 💵 **Cash (40%):** **161.67 €**
    * 💎 **Tokens (60%):** **242.50 €**

*Забележка: Сумата е по-ниска от Протокол 3, защото тук инфраструктурата (BIND/UFW) вече е готова и се фокусираме само върху хостинг слоя на конкретния домейн.*
