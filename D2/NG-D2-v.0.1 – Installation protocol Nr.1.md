...
## Етап 1, Стъпка 15: ФИНАЛНА ПРОВЕРКА (AUDIT & HARDENING VALIDATION)

**🎯 Цел**
Пълна верификация на системната сигурност, мрежовите портове и административния достъп. Уверяваме се, че няма „задни врати“ и че само планираните услуги са видими за външния свят.

**15.1. Одит на отворените портове (Вътрешен)**

Проверка на кои портове слуша сървърът в момента:

```bash
sudo ss -tulpn | grep LISTEN
```

***Очакван резултат:*** Трябва да виждате само порт `59623` (SSH), `80` (Nginx) и услугите на MariaDB/Netdata, ако са конфигурирани да слушат външно. Порт `22` трябва да отсъства напълно.

**15.2. Външно сканиране (Nmap тест)**

Изпълнете това от вашия iMac (локално), за да видите какво вижда един потенциален хакер:

```bash
nmap -p- 65.108.12.147
```

***Очакван резултат:*** Всички портове освен дефинираните от вас трябва да са в статус `closed` или `filtered`.

**15.3. Валидация на забраната за Root и Парола**

Опит за директен вход като root (трябва да бъде отхвърлен веднага):

```bash
ssh -p 59623 root@65.108.12.147
```

Опит за вход с парола за потребителя `robinzon`:

```bash
ssh -o PreferredAuthentications=password -p 59623 robinzon@65.108.12.147
```

***Очакван резултат PASS:*** И двете команди трябва да върнат `Permission denied (publickey)`.

**15.4. Проверка на Sudo правата без парола**

Влезте като `robinzon` и изпълнете:

```bash
sudo whoami
```

***Очакван резултат PASS:*** Командата трябва да върне `root`, без да ви пита за парола (поради `NOPASSWD` настройката).

**15.5. Проверка на статуса на защитните стени**
...
***Очакван резултат:*** UFW може все още да е `inactive` (ако чакате Етап 2), но Fail2Ban трябва да е `active` и да следи SSH лога на новия порт.

**15.6. Системен интегритет**

Проверка за грешки в системния журнал след промените:

```bash
sudo journalctl -p err..alert -n 20
```

***Очакван резултат:*** Липса на критични грешки, свързани с SSH или системните демони.

---

**PASS (Стъпка 15):**
```text
[OK] SSH Port Hardening Verified
[OK] Root Login Disabled
[OK] Password Authentication Blocked
[OK] Admin Sudo Access Verified
[OK] External Port Audit Passed
```

---

## ОТЧЕТ НА ИЗПЪЛНЕНИТЕ ЗАДАЧИТЕ
...
| **Стъпка 14**       | Забрана на вход с парола (критична конфигурация и тест) | **Level A** |    15       |
| **Стъпка 15**       | Финална проверка (Скенер на портове и системен одит)    | **Level A** |    20       | 
...
| :--- | :---: | :---: | :---: | :---: |
| **I. Базова инсталация** | **Level C** | **30** | **100.00** | **50.00** |
| **II. Администрация и стек** | **Level B** | **75** | **150.00** | **187.50** |
| **III. Сигурност и Hardening** | **Level A** | **70** | **250.00** | **291.67** |
| **ОБЩО ЗА ПРОТОКОЛА:** | | **175 мин.** | **Средна: 181.43** | **529.17 €** |
