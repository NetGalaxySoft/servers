...
# =====================================================================
# [МОДУЛ 2] ИНСТАЛИРАНЕ НА WIREGUARD И ДОПЪЛНИТЕЛНИ ИНСТРУМЕНТИ
# =====================================================================
echo "[2] ИНСТАЛИРАНЕ НА WIREGUARD И ИНСТРУМЕНТИ..."
echo "-----------------------------------------------------------"
echo ""
...
if sudo grep -q '^VPN_RESULT_MODULE2=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  # ✅ Инсталиране на qrencode (за QR кодове при клиентска конфигурация)
  echo "🔍 Инсталиране на qrencode за мобилни QR кодове..."
  if ! sudo apt-get install -y qrencode; then
    echo "⚠️ Инсталирането на qrencode се провали. QR кодове няма да бъдат достъпни."
  else
    echo "✅ qrencode е инсталиран успешно."
  fi
  echo ""

  # ✅ Проверка за наличието на командата wg
...
if grep -q '^VPN_RESULT_MODULE3=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  ...
  echo "ℹ️ IP на сървъра: $SERVER_IP"
  echo ""
  ...
  # --- Потвърждение с валидна проверка ---
  echo ""
  ...
  while true; do
    ...
    case "$confirm" in
      [Yy]*) break ;;
      [Nn]*) echo "❎ Скриптът е прекратен от потребителя."; exit 0 ;;
...
# =====================================================================
echo "[4] ФИНАЛИЗИРАНЕ НА ИНСТАЛАЦИЯТА..."
echo "-----------------------------------------------------------"
echo ""
...
echo "▶ Проверка на статуса на WireGuard..."
if systemctl is-active --quiet wg-quick@wg0; then
  echo "✅ WireGuard интерфейсът (wg0) е активен."
else
  echo "❌ Грешка: WireGuard не е активен!"
  echo "Опитайте да стартирате ръчно: systemctl start wg-quick@wg0"
  exit 1
fi
echo ""

# --- Извличане на данни за отчет ---
SERVER_IP=$(grep '^SERVER_IP=' "$MODULES_FILE" | awk -F'=' '{print $2}' | tr -d '"')
VPN_SUBNET=$(grep '^VPN_SUBNET=' "$MODULES_FILE" | awk -F'=' '{print $2}' | tr -d '"')
VPN_SERVER_IP=$(grep '^VPN_SERVER_IP=' "$MODULES_FILE" | awk -F'=' '{print $2}' | tr -d '"')
WG_PORT=$(grep '^WG_PORT=' "$MODULES_FILE" | awk -F'=' '{print $2}' | tr -d '"')

# --- Обобщение ---
echo "📋 Обобщение на VPN конфигурацията:"
echo "----------------------------------------"
echo "Публичен IP на сървъра:  $SERVER_IP"
echo "VPN подмрежа:            $VPN_SUBNET"
echo "VPN IP на сървъра:       $VPN_SERVER_IP"
echo "WireGuard порт:          $WG_PORT"
echo "Конфигурационен файл:    $WG_CONF"
echo "----------------------------------------"
echo ""
  ...
  case "$confirm" in
    [Yy]*)
      ...
      if grep -q '^SETUP_VPS_VPN_STATUS=' "$SETUP_ENV_FILE" 2>/dev/null; then
        sed -i 's|^SETUP_VPS_VPN_STATUS=.*|SETUP_VPS_VPN_STATUS=✅|' "$SETUP_ENV_FILE"
      else
        echo "SETUP_VPS_VPN_STATUS=✅" >> "$SETUP_ENV_FILE"
      fi
