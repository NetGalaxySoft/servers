...
if [[ -z "$SERVER_IP" ]]; then
  echo "❌ Неуспешно извличане на публичен IP."
  exit 1
fi
...
echo ""
echo ""
echo "⬇️ За да свалите файловете на локалния компютър, изпълнете от своя терминал:"
if [[ -f "$CLIENTS_DIR/$CLIENT_NAME.png" ]]; then
  echo "scp root@$SERVER_IP:$CLIENT_CONF . && scp root@$SERVER_IP:$CLIENTS_DIR/$CLIENT_NAME.png ."
else
