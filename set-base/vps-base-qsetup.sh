...
if [[ "$SSH_PORT" != "$CURRENT_SSH_PORT" ]]; then
  echo "🔧 Актуализиране на /etc/ssh/sshd_config (без рестарт на услугата)..."
  if grep -q "^#*Port " /etc/ssh/sshd_config; then
    sudo sed -i "s/^#*Port .*/Port $SSH_PORT/" /etc/ssh/sshd_config
  else
    echo "Port $SSH_PORT" | sudo tee -a /etc/ssh/sshd_config > /dev/null
  fi
  echo "ℹ️ Промяната ще влезе в сила след финалното одобрение и рестарта (модул 11)."
else
  echo "ℹ️ Няма промяна – SSH портът остава $SSH_PORT."
fi

# --- Настройка на UFW за новия порт (без активиране на UFW) ---
if ! sudo ufw status | grep -q "$SSH_PORT/tcp"; then
  echo "➕ Добавяне на правило за SSH порт $SSH_PORT (UFW е в неактивен режим)..."
