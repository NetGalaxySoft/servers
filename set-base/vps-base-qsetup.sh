...
# Проверка за активни правила на iptables (без да премахваме пакета)
...
# --- Тест на UFW ---
echo "🔍 Тест на UFW (временно активиране)..."

# Разрешаване на реалния SSH порт преди активиране
if ! sudo ufw allow "$CURRENT_SSH_PORT"/tcp >/dev/null 2>&1; then
  echo "ℹ️ Добавяне на SSH порт ($CURRENT_SSH_PORT) за тест..."
fi

# Временно активиране за тест
if ! sudo ufw --force enable >/dev/null 2>&1; then
  echo "❌ UFW не може да бъде активиран. Скриптът ще бъде прекратен."
  exit 1
fi

# Проверка дали правилото работи
if ! sudo ufw status | grep -q "$CURRENT_SSH_PORT"; then
  echo "❌ UFW не успя да добави SSH порт ($CURRENT_SSH_PORT)."
  echo "➡ Сървърът не може да бъде конфигуриран коректно."
  exit 1
fi

# Деактивиране след тест (оставяме активирането за последния модул)
sudo ufw disable >/dev/null 2>&1
echo "✅ Тестът на UFW приключи успешно. Защитната стена ще бъде активирана по-късно."
