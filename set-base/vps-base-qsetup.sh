...
# === [ПРЕДВАРИТЕЛНА ПРОВЕРКА НА ОПЕРАЦИОННАТА СИСТЕМА] =========================
...
if sudo grep -q "^RESULT_BASE_TOOLS=✅" "$SETUP_ENV_FILE"; then
  ...
  echo "RESULT_BASE_TOOLS=$RESULT_BASE_TOOLS" | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
fi
echo ""
echo ""
...
if sudo grep -q "^RESULT_LOCALES=✅" "$SETUP_ENV_FILE"; then
  ...
  if sudo locale-gen && sudo update-locale; then
    ...
    echo "RESULT_LOCALES=✅" | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
  else
...
# ✅ Запис на чувствителни данни за следващи модули
echo "ADMIN_USER=\"$ADMIN_USER\"" | sudo tee -a "$MODULES_FILE" > /dev/null

# ✅ Запис в setup.env само при пълен успех
echo "RESULT_ADMIN_USER=✅" | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
...
if ! command -v ufw >/dev/null 2>&1; then
  ...
  if ! sudo apt-get update || ! sudo apt-get install -y ufw; then
    echo "❌ Грешка при инсталацията на UFW! Скриптът не може да продължи."
    exit 1
  fi
else
  echo "✅ UFW е вече инсталиран."
fi

# --- Проверка дали ufw allow работи ---
echo "🔍 Тест на UFW..."
if ! sudo ufw allow 22/tcp >/dev/null 2>&1; then
  echo "❌ UFW не може да добавя правила (problem running)."
  echo "➡ Без работеща конфигурация на UFW сървърът не може да бъде защитен."

  # Въпрос към оператора
  while true; do
    echo "Изберете действие:"
    echo "[1] Автоматично преинсталиране на ufw и iptables"
    echo "[2] Прекратяване на скрипта за ръчно конфигуриране"
    read -rp "Вашият избор (1/2): " choice

    case "$choice" in
      1)
        echo "🔄 Преинсталиране на ufw и iptables..."
        if sudo apt-get install --reinstall -y ufw iptables; then
          echo "✅ UFW и iptables бяха преинсталирани."
          echo "🔍 Повторен тест на UFW..."
          if ! sudo ufw allow 22/tcp >/dev/null 2>&1; then
            echo "❌ Дори след преинсталация UFW не работи. Скриптът ще бъде прекратен."
            exit 1
          else
            echo "✅ UFW работи коректно след преинсталация."
            sudo ufw delete allow 22/tcp >/dev/null 2>&1
            break
          fi
        else
          echo "❌ Грешка при преинсталацията. Скриптът ще бъде прекратен."
          exit 1
        fi
        ;;
      2)
        echo "⛔ Скриптът беше прекратен по искане на оператора."
        exit 1
        ;;
      *)
        echo "❌ Невалиден избор. Моля, въведете 1 или 2."
        ;;
    esac
  done
else
  # Ако тестът е успешен, премахваме тестовото правило
  sudo ufw delete allow 22/tcp >/dev/null 2>&1
fi

# ✅ Запис на информация за firewall в todo.modules
echo "FIREWALL_SYSTEM=ufw" | sudo tee -a "$MODULES_FILE" > /dev/null
...
if ! sudo ufw allow "$CURRENT_SSH_PORT"/tcp; then
  echo "❌ Неуспешно добавяне на SSH порт към правилата. Скриптът не може да продължи."
  exit 1
fi

# --- Въвеждане на допълнителни портове ---
echo ""
echo "🔧 ВЪВЕДЕТЕ ДОПЪЛНИТЕЛНИ ПОРТОВЕ ЗА ОТВАРЯНЕ (ENTER за продължение, 'q' за изход)"
PORT_LIST=()
...
while true; do
  ...
  if [[ "$port" == "q" || "$port" == "Q" ]]; then
    ...
    if sudo ufw allow "$port"/tcp; then
      ...
      echo "❌ Неуспешно добавяне на порт $port. Скриптът не може да продължи."
      exit 1
    fi
  fi
done

# ✅ Запис на портовете в todo.modules
echo "PORT_LIST=\"${PORT_LIST[*]}\"" | sudo tee -a "$MODULES_FILE" > /dev/null
...
# ✅ Запис на доверените мрежи в todo.modules
echo "TRUSTED_NETS=\"${TRUSTED_NETS[*]}\"" | sudo tee -a "$MODULES_FILE" > /dev/null

# ✅ Запис в setup.env само при успех
echo "RESULT_TRUSTED_NETS=✅" | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
...
# ✅ Запис на SSH порта в todo.modules (за следващи модули)
echo "SSH_PORT=\"$SSH_PORT\"" | sudo tee -a "$MODULES_FILE" > /dev/null

# ✅ Запис в setup.env (само при успех)
echo "RESULT_SSH_PORT=✅" | sudo tee -a "$SETUP_ENV_FILE" > /dev/null
...
[[ -z "$FIREWALL_SYSTEM" ]] && FIREWALL_SYSTEM="❔ не е зададена"
...
while true; do
  ...
  case "$final_confirm" in
    ...
    [Nn])
      ...
      # ❌ Премахваме записа SETUP_VPS_BASE_STATUS=❌ (не добавяме нищо)
      exit 0
      ;;
    *)
