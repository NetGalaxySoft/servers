...
# =====================================================================
# [МОДУЛ 1] ПРЕДВАРИТЕЛНИ ПРОВЕРКИ И ИНИЦИАЛИЗАЦИЯ (БЕЗ ЗАПИСИ)
# =====================================================================
echo "[1] ПРЕДВАРИТЕЛНИ ПРОВЕРКИ НА СИСТЕМАТА..."
echo "-----------------------------------------------------------"
echo ""
...
# ✅ Базова конфигурация трябва да е завършена и защитена
if [[ ! -f "$SETUP_ENV_FILE" ]] || ! sudo grep -q '^SETUP_VPS_BASE_STATUS=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  echo "🛑 Сървърът няма валидна базова конфигурация (липсва SETUP_VPS_BASE_STATUS=✅)."
  echo "   Стартирайте първо vps-base-qsetup.sh и опитайте отново."
  exit 1
fi

# ✅ Скриптът за мониторинг не трябва да е изпълняван успешно преди
if sudo grep -q '^SETUP_VPS_MONITORING_STATUS=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  echo "🛑 Мониторинг стекът вече е инсталиран на тази машина (SETUP_VPS_MONITORING_STATUS=✅)."
  exit 0
fi
if sudo grep -q '^MON_RESULT_MODULE1=✅' "$SETUP_ENV_FILE" 2>/dev/null; then
  echo "ℹ️ Модул 1 вече е отбелязан като изпълнен. Продължаваме по-нататък..."
else
  echo "ℹ️ Модул 1 предстои да бъде маркиран след успешни проверки."
fi

# ✅ todo.modules НЕ трябва да съществува (той е временен файл от базовия скрипт)
if [[ -f "$MODULES_FILE" ]]; then
  echo "🛑 Открит е временният файл $MODULES_FILE."
  echo "   Това означава, че базовият скрипт не е приключил коректно (или защитата не е приложена)."
  echo "   Моля, довършете базовата инсталация и опитайте отново."
  exit 1
fi
...
# --- До тук НЯМА записи. Ако всичко е наред, разрешаваме записите и маркираме модул 1.
enable_writes
mark_success "MON_RESULT_MODULE1"
ok "[1] Проверки успешни. Модул 1 е маркиран като завършен."
echo ""
